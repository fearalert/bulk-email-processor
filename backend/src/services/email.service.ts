...
export class EmailService {
  ...
  public async queueBulkEmails(
    ...
    logger.info(`Starting bulk email processing`, `userId=${userId}, totalEmails=${emails.length}, templateId=${templateId}`);
    ...
    try {
      ...
      if (invalidEmails.length > 0) {
        logger.warn(`Skipped invalid emails`, `userId=${userId}, invalidCount=${invalidEmails.length}, invalidEmails=${invalidEmails.join(', ')}`);
      }
      ...
      const tasks = validEmails.map(email =>
        limit(async () => {
          try {
            ...
            logger.debug(`Queued email`, `userId=${userId}, logId=${log.id}, email=${trimmedEmail}`);
            return { success: true, email: trimmedEmail, logId: log.id };
          } catch (err: any) {
            logger.error(`Failed to queue email`, `userId=${userId}, email=${email}, error=${err.message || err}`);
      ...
      logger.error(`Bulk email processing failed`, `userId=${userId}, templateId=${templateId}, error=${err.message || err}`);
      throw err;
    }
  }
  ...
  private async validateUserAndTemplate(userId: number, templateId: number) {
    try {
      ...
      if (err.message.includes('Template with id')) {
        ...
        throw new AppError(`Invalid template: ${err.message}. Please create a template first or use an existing template ID.`, 400);
      }
      throw err;
    }
  }
